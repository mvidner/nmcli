...
def package_to_row(yum, pkg, no_abbrev, key_list):
    ...
    for key in key_list:
        ...
        if key == "installed":
            ...
            installed_list = yum.rpmdb.searchNevra(name=pkg.name, arch=pkg.arch)
            if len(installed_list) > 0:
                val = 'v'
                
                for installed in installed_list:
                    if installed.pkgtup == pkg.pkgtup:
                        val = 'i'
                        break

        elif key == "repo":
            val = pkg.returnSimple('repoid')

        elif key == "version":

            if no_abbrev:
                val = naevr_to_str(pkg.pkgtup)
            else:
                val = naevr_to_abbrev_str(pkg.pkgtup)
