...
default_opt_table = [
    ...
    ["",  "ignore-rc-file", "",   "Don't read nm's startup file (~/.nmrc)"],
    ["",  "ignore-env", "",       "Ignore the NM_ARGS environment variable"],
...
def get_user_default_args(argv, command_name):
    ...
    # Try to read the .nmrc file.  It basically works like a .cvsrc file.
    if "--ignore-rc-file" not in argv:
        nmrc_files = (os.path.expanduser("~/.nmrc"),
    ...
    if "--ignore-env" not in argv:
        if os.environ.has_key("NM_ARGS"):
            args = string.split(os.environ["NM_ARGS"])
